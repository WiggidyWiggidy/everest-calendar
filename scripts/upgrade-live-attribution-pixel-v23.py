...
def patch(pixel):
    original=pixel
    pixel=pixel.replace('data-pixel-version="2.0"','data-pixel-version="2.3"',1).replace('data-pixel-version="2.1"','data-pixel-version="2.3"',1).replace('data-pixel-version="2.2"','data-pixel-version="2.3"',1)
...
"""
    ...
    permalink_helpers="""  function sendBeforeNavigation(eventName, extra) {
    ...
    try { fetch(EL_API, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), keepalive: true, mode: 'no-cors' }).catch(function() {}); } catch(e) {}
  }
  function trackCartPermalink(href) {
    var match = String(href || '').match(/\/cart\/(\d+):(\d+)/);
    if (!match) return false;
    var variantId = match[1];
    var quantity = parseInt(match[2], 10) || 1;
    var extra = { shopify_variant_id: variantId, quantity: quantity, event_properties: { tracking_source: 'cart_permalink_click' } };
    sendBeforeNavigation('cart_add_request', extra);
    var now = Date.now();
    if (now - lastAtcAt >= 2500) { lastAtcAt = now; sendBeforeNavigation('product_added_to_cart', extra); }
    sendBeforeNavigation('checkout_started', extra);
    return true;
  }
"""
    if 'function trackCartPermalink(href)' not in pixel:
        click_marker="  document.addEventListener('click', function(e) {\n"
        if click_marker not in pixel: raise SystemExit('click marker missing for cart permalink helper')
        pixel=pixel.replace(click_marker,permalink_helpers+click_marker,1)
    pixel=pixel.replace("    if (/add to cart|order now|buy now|reserve now/.test(text)) verifyCartDelta('cta_cart_delta');", "    var isCartPermalink = trackCartPermalink(href);\n    if (!isCartPermalink && /add to cart|order now|buy now|reserve now/.test(text)) verifyCartDelta('cta_cart_delta');")
    return pixel
...
for key in keys:
    ...
    reread=get(key)
    assert 'data-pixel-version="2.3"' in reread and 'window.__el_xhr_patched' in reread and "function trackCartPermalink(href)" in reread and "sendBeforeNavigation('checkout_started', extra)" in reread
