...
async function fetchGraphInsights(token: string, accountId: string, since: string, until: string) {
  const fields = 'account_id,account_name,campaign_id,campaign_name,adset_id,adset_name,ad_id,ad_name,date_start,impressions,clicks,spend,ctr,cpc,cpm,reach,frequency,actions,action_values,purchase_roas';
  ...
  while (nextUrl) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Meta Insights API ${response.status}: ${detail}`);
    }
    const payload: InsightsPayload = await response.json();
...
export async function POST(request: NextRequest) {
  try {
    ...
    const allRows = await fetchGraphInsights(metaToken, adAccountId, iso(sinceDate), iso(untilDate));
    ...
    for (const row of allRows) {
      ...
      synced++;
      const account = accountByDate.get(row.date_start) ?? { spend: 0, impressions: 0, clicks: 0, purchases: 0, revenue: 0 };
      account.spend += spend;
      account.impressions += impressions;
      account.clicks += clicks;
      account.purchases += purchases;
      account.revenue += revenue;
      accountByDate.set(row.date_start, account);
    }
    ...
    for (const [date, account] of Array.from(accountByDate.entries())) {
      const { error: summaryErr } = await supabase
        ...
        .upsert({
          user_id: auth.userId || DEFAULT_USER_ID,
          date,
